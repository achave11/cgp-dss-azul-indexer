...
# json_str = urlopen(
#     requestConstructor(
#         str("https://metadata."+redwood_host+"/entities?page=0"),
#         headers),
#     context=ctx).read()
...
def parseResultEntry(result_entry):
    ...
    bundle_id = result_entry['bundle_fqid']  # changed on 2018-01-03 (MK)
...
def main():
    ...
    for bundle, version in bundle_list:
        data = json.dumps({"query":
                           {"query":
                             {"match_all":{}}},
                           "subscription_id": "ba50df7b-5a97-4e87-b9ce-c0935a817f0b",
                           "transaction_id": "ff6b7fa3-dc79-4a79-a313-296801de76b9",
                           "match": {"bundle_version": version,
                                     "bundle_uuid": bundle}})
